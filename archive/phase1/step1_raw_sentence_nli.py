"""
Phase 2 Step 1: Raw Sentence-Level NLI Output

PURPOSE: See what the model actually produces on individual sentences
compared to paragraph-level. No aggregation, no filtering, no decisions.
Just raw data to inform all subsequent design choices.

For each test sample:
  1. Run paragraph-level NLI (baseline, same as Phase 1)
  2. Split into sentences (nltk)
  3. Run NLI on each sentence independently
  4. Show full [S, N, O] probability distributions for everything

WHAT TO LOOK FOR IN THE OUTPUT:
  - Multi-sentence samples: does splitting expose a clear opposing sentence
    that the paragraph-level missed?
  - Single-sentence samples: does sentence-level match paragraph-level?
    (it should, since there's only 1 sentence)
  - Weak negation samples: what are the exact probability distributions?
    How close is P(opposing) to the threshold?

Usage: python step1_raw_sentence_nli.py
Output: tests/step1_raw_output.txt
"""
...
def nli(premises: list[str], hypotheses: list[str]) -> list[dict]:
    """Batched NLI. Returns [{label, conf, p_supp, p_neut, p_opp}, ...]"""
    if not premises:
        return []
    inputs = tokenizer(
        premises, hypotheses,
...
# TEST DATA
# ============================================================
LABEL_NAMES = {0: "supporting", 1: "neutral", 2: "opposing"}
...
def main():
    out = []
    out.append(f"Phase 2 Step 1: Raw Sentence-Level NLI Output")
    ...
    out.append(f"Samples: {len(TEST_DATA)}")
    out.append(f"")
    out.append(f"LEGEND:")
    out.append(f"  PARA  = paragraph-level NLI (full source text as one input)")
    out.append(f"  S1/S2 = individual sentence NLI")
    out.append(f"  [S:__ N:__ O:__] = P(supporting), P(neutral), P(opposing)")
    out.append(f"  Samples marked MULTI have >1 sentence (splitting matters)")
    out.append(f"  Samples marked SINGLE have 1 sentence (splitting = no-op)")
    out.append(f"")
    out.append("=" * 100)
    ...
    para_correct = 0
    multi_count = 0
    pattern_stats = {}  # pattern -> {para_correct, total}

    for i, item in enumerate(TEST_DATA):
        source = item["source"]
        claim = item["claim"]
        expected = LABEL_NAMES[item["label"]]
        pattern = item["pattern"]

        if pattern not in pattern_stats:
            pattern_stats[pattern] = {"para_correct": 0, "total": 0}
        pattern_stats[pattern]["total"] += 1

        # Paragraph-level
        para = nli([source], [claim])[0]
        para_ok = para["label"] == expected
        if para_ok:
            para_correct += 1
            pattern_stats[pattern]["para_correct"] += 1
        ...
        # Run NLI on each sentence
        if sentences:
            sent_results = nli(sentences, [claim] * len(sentences))
        else:
            sent_results = []

        # Output
        multi_tag = "MULTI" if is_multi else "SINGLE"
        para_tag = "CORRECT" if para_ok else "WRONG"

        out.append(f"")
        out.append(f"--- Sample {i+1:3d}/{total} | {pattern} | expected: {expected} | {multi_tag} ---")
        out.append(f"CLAIM: {claim}")
        out.append(f"SOURCE: {source}")
        out.append(f"")
        out.append(f"  PARA: {para['label']:10s} [S:{para['p_supp']:.4f} N:{para['p_neut']:.4f} O:{para['p_opp']:.4f}] conf={para['conf']:.4f}  {para_tag}")

        if is_multi:
            for j, (sent, sr) in enumerate(zip(sentences, sent_results)):
                wc = len(sent.split())
                sent_ok = "CORRECT" if sr["label"] == expected else ""
                out.append(f"  S{j+1}:   {sr['label']:10s} [S:{sr['p_supp']:.4f} N:{sr['p_neut']:.4f} O:{sr['p_opp']:.4f}] conf={sr['conf']:.4f}  {sent_ok}  [{wc}w] \"{sent}\"")

            # Quick comparison: did ANY sentence get the right answer when paragraph didn't?
            if not para_ok:
                correct_sents = [j+1 for j, sr in enumerate(sent_results) if sr["label"] == expected]
                if correct_sents:
                    out.append(f"  >>> SENTENCE-LEVEL WINS: sentences {correct_sents} got it right, paragraph got it wrong")
                else:
                    out.append(f"  >>> BOTH LOSE: neither paragraph nor any individual sentence got the right answer")
            else:
                wrong_sents = [j+1 for j, sr in enumerate(sent_results) if sr["label"] != expected]
                if wrong_sents:
                    out.append(f"  >>> NOTE: paragraph correct, but sentences {wrong_sents} are wrong (aggregation matters)")
        else:
            # Single sentence: just confirm it matches paragraph
            if sent_results:
                sr = sent_results[0]
                match = "MATCHES PARA" if sr["label"] == para["label"] else "DIFFERS FROM PARA (!)"
                out.append(f"  S1:   {sr['label']:10s} [S:{sr['p_supp']:.4f} N:{sr['p_neut']:.4f} O:{sr['p_opp']:.4f}] conf={sr['conf']:.4f}  {match}")
    ...
    out.append(f"Multi-sentence samples: {multi_count}")
    out.append(f"Single-sentence samples: {total - multi_count}")
    out.append(f"Paragraph-level accuracy: {para_correct}/{total} ({100*para_correct/total:.1f}%)")
    out.append(f"")
    out.append(f"Per-pattern paragraph-level accuracy:")
    for p in sorted(pattern_stats.keys()):
        s = pattern_stats[p]
        out.append(f"  {p:25s}: {s['para_correct']}/{s['total']} ({100*s['para_correct']/s['total']:.1f}%)")
    out.append("")

    # Write
    os.makedirs("tests", exist_ok=True)
    path = "tests/step1_raw_output.txt"
    with open(path, "w") as f:
        f.write("\n".join(out))
    print(f"\nOutput written to {path}")
    print(f"Paragraph baseline: {para_correct}/{total}")
    print(f"Multi-sentence samples: {multi_count}")
    print(f"\nOpen {path} and look at:")
    print(f"  1. MULTI samples: do individual sentences expose better signal?")
    print(f"  2. weak_negation: how close is P(opposing) to P(neutral)?")
    print(f"  3. Any 'DIFFERS FROM PARA' on single-sentence samples (shouldn't happen)")
